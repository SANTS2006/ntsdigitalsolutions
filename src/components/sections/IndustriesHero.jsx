  ...
  <section className="relative overflow-hidden bg-slate-950 text-white">
    ...
    <div className="relative mx-auto max-w-7xl px-5 py-20 lg:px-6 lg:py-28">
      <div className="grid gap-12 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
        ...
        <div className="grid grid-cols-2 gap-3">{focus.map(([Icon, label]) => <div key={label} className="rounded-2xl border border-white/10 bg-white/[.06] p-5 backdrop-blur"><Icon className="text-xl text-blue-300" /><p className="mt-8 font-bold">{label}</p></div>)}</div>
      </div>
    </div>
  </section>
);
