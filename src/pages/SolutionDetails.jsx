    ...
    return <>
        ...
        <section className="bg-slate-950 py-24 text-white">
            <div className="mx-auto max-w-7xl px-5 lg:px-6">
                ...
                <div className="mt-12 max-w-4xl"><p className="text-xs font-bold uppercase tracking-[.22em] text-blue-300">{solution.category}</p><h1 className="mt-4 text-4xl font-extrabold tracking-tight md:text-6xl">{solution.title}</h1><p className="mt-6 max-w-3xl text-lg leading-8 text-slate-300">{solution.description}</p></div>
            </div>
        </section>

        <section className="py-20"><div className="mx-auto max-w-7xl px-5 lg:px-6"><div className="grid gap-6 md:grid-cols-3"><div className="rounded-2xl border border-slate-200 bg-slate-50 p-6"><p className="text-xs font-bold uppercase tracking-widest text-slate-400">The challenge</p><p className="mt-3 leading-7 text-slate-600">{solution.challenge}</p></div><div className="rounded-2xl border border-blue-100 bg-blue-50 p-6"><p className="text-xs font-bold uppercase tracking-widest text-blue-500">NTS approach</p><p className="mt-3 leading-7 text-slate-700">{solution.solution}</p></div><div className="rounded-2xl border border-slate-200 bg-slate-950 p-6 text-white"><FiLayers className="text-2xl text-blue-300" /><p className="mt-4 text-xs font-bold uppercase tracking-widest text-slate-500">Built with purpose</p><p className="mt-3 leading-7 text-slate-300">A focused system designed around the users, workflows, and operational needs of its intended environment.</p></div></div></div></section>
